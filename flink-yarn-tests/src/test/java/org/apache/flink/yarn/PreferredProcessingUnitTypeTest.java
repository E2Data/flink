...
//		configuration.setString(CoreOptions.FLINK_JVM_OPTIONS, "-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=5005");

		final YarnClient yarnClient = getYarnClient();
		YarnConfiguration yarnConfiguration = getYarnConfiguration();
//		yarnConfiguration.set("yarn.log-aggregation-enable", "true");
	 ...
	 */
	@Test
	public void testDemandResourceSpecification() throws FileNotFoundException, ProgramInvocationException, ClusterDeploymentException, ExecutionException, InterruptedException {
		File testingJar = getTestJarPath("BatchWordCount.jar");

		HashMap<String, Resource> extendedResources = new HashMap<>();
		extendedResources.put("accelerator.name", new AcceleratorResource("yarn.io/gpu-geforce1080gtx"));
		ResourceSpec demandResource = new ResourceSpec(ProcessingUnitType.GPU, 0, 0, 0, 0, 0, extendedResources);
